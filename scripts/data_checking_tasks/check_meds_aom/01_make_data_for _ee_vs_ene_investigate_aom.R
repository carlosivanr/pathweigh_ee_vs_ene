...
# The tricky part is that some meds have multiple names and dosages and part
# of the reason they need to be collapsed


# Subset the meds table to those that have an ActiveMed and their Arb_PersonId is seq_df2. In baseline_characteristics, seq_df 2 consisted of only those that were eligible and enrolled. 
meds_sub <- meds %>% 
  filter(ActiveMed == "Y", 
         Arb_PersonId %in% seq_df2$Arb_PersonId) %>%
  mutate_at(c("MedicationName", "GenericName"), tolower)

# Create meds_ref, which will contain information on whether or not a medication is to be classified as a wieght gain, weight loss, or anti obesity medication (AOM).
meds_ref <- read.xlsx(
...
# Merge meds_list with meds_ref via left join, then
# merge that new data frame with meds_sub via right join,
# then keep only the columns of interest
meds_full <- meds_list %>% 
...
# filter out the meds_AOM data frame and then merge it with each unique visit in seq_df2
meds_AOM <- meds_full %>% 
  filter(AOM == 1) %>%
  left_join(., distinct(seq_df2, Arb_PersonId, IndexDate), by = "Arb_PersonId")

#Subset to the time period of interest (baseline period)
# *** This should be moved up right after making meds_full so then you wouldn't have to do the same filtering to med_aom
meds_full <- meds_full %>%
...
# Keep only the distinct Arb_PersonId, IndexDate, and UniqueMedNames
# n.b. at this point there will be duplicate Arb_PersonIds in the dataframe
meds_AOM <- meds_AOM %>% 
  distinct(Arb_PersonId, IndexDate, uniqueMedName)
...
# This is where we will get only the 6 unique AOM meds
meds_AOM <- meds_AOM %>%
  left_join(., distinct(seq_df2, Arb_PersonId, Cohort), by = "Arb_PersonId")


# join the meds counts
#*** I don't think the square brackets are necessary for this one
...
rm(meds_list, meds_full, meds_ref, meds_counts, meds_AOM)
